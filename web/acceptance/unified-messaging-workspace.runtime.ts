...
async function execute(world: World, text: string) {
  ...
  if (text === 'an authorized operator previews an ineligible reply') { await openConversation(world, 'ineligible'); await page.getByLabel(/Reply to .* directly/).fill('Preserved draft'); await page.getByRole('button', { name: 'Preview reply' }).click(); return }
  if (text === 'actionable eligibility guidance is shown and the draft remains') { await expect(page.getByText(/opted out/)).toBeVisible(); await expect(page.getByLabel(/Reply to .* directly/)).toHaveValue('Preserved draft'); return }
  if (text === 'an authorized operator previews a valid individual reply') { await openConversation(world, 'normal'); await page.getByLabel(/Reply to .* directly/).fill('Thanks'); await page.getByRole('button', { name: 'Preview reply' }).click(); return }
